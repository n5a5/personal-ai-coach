...
export default function CoachPage() {
  ...
  async function send(text=input){
    ...
    const next=[...messages,{role:"user" as const,content:clean}]; setMessages(next);
    try {
      ...
      else if(data.setup){setSetup(true);} else if(data.message){setMessages(m=>[...m,{role:"assistant",content:data.message}]);}
    } catch { setError("I couldn't reach the coach. Please try again."); }
    finally { setLoading(false); }
  }

  if(pageLoading) return <main style={{maxWidth:850,margin:"0 auto",padding:"40px 20px",minHeight:"100vh"}}><button onClick={()=>router.push("/")} style={{border:0,background:"transparent",padding:0}}>← Today</button><p style={{marginTop:30,opacity:.6}}>Loading your coach…</p></main>;

  return <main style={{maxWidth:850,margin:"0 auto",padding:"28px 20px 40px",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
    <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}><div><button onClick={()=>router.push("/")} style={{border:0,background:"transparent",padding:0}}>← Today</button><h1 style={{margin:"8px 0 0",fontSize:34}}>Talk to your coach.</h1><p style={{margin:"5px 0 0",opacity:.65}}>Built around your goals, values and the person you're trying to become.</p></div><button onClick={()=>router.push("/profile")} style={{border:"1px solid #ddd",background:"white",borderRadius:10,padding:"9px 12px"}}>Coach profile</button></header>

    {messages.length===0 && <section style={{background:"white",borderRadius:20,padding:22,marginBottom:12}}><div style={{fontSize:13,fontWeight:700,letterSpacing:1,textTransform:"uppercase",opacity:.5}}>Your starting point</div><h2 style={{margin:"7px 0 8px"}}>You don't need to solve everything today.</h2><p style={{margin:"0 0 15px",lineHeight:1.5,opacity:.75}}>We'll focus on what you can control, separate facts from predictions, take the next useful action, and then get you back to living your life. When you're anxious, we can slow things down instead of letting the second arrow make things worse.</p><div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8}}>{starters.map(([label,prompt])=><button key={label} onClick={()=>send(prompt)} style={{textAlign:"left",padding:12,border:"1px solid #e1dfda",background:"#faf9f6",borderRadius:12,cursor:"pointer"}}><b>{label}</b><div style={{fontSize:12,opacity:.6,marginTop:4}}>{prompt}</div></button>)}</div></section>}
    ...
    <form onSubmit={e=>{e.preventDefault();send()}} style={{display:"flex",gap:10,marginTop:10}}><input value={input} onChange={e=>setInput(e.target.value)} placeholder="Tell your coach what’s on your mind…" style={{flex:1,padding:14,border:"1px solid #ddd",borderRadius:12}}/><button disabled={loading} style={{border:0,borderRadius:12,padding:"0 20px",background:"#171717",color:"white",fontWeight:700}}>{loading?"Thinking…":"Send"}</button></form>
  </main>;
}

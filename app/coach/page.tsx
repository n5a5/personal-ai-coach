...
export default function CoachPage() {
  ...
  return (
    <main style={{ maxWidth: 850, margin: "0 auto", padding: "24px 16px 40px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
        <div><button onClick={() => router.push("/")} style={{ border: 0, background: "transparent", padding: 0, fontSize: 14 }}>← Today</button><h1 style={{ margin: "8px 0 0", fontSize: "clamp(28px,7vw,34px)" }}>Talk to your coach.</h1><p style={{ margin: "5px 0 0", opacity: .65 }}>Built around your goals, values and the person you're becoming.</p></div>
      ...
      {messages.length === 0 && <section style={{ background: "white", borderRadius: 20, padding: 20, marginBottom: 12 }}><div style={{ fontSize: 13, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", opacity: .5 }}>Your starting point</div><h2 style={{ margin: "7px 0 8px" }}>I'm here. What's going on?</h2><p style={{ margin: "0 0 15px", lineHeight: 1.5, opacity: .75 }}>We'll focus on what you can control, separate facts from predictions, take the next useful action, and then get you back to living your life.</p><div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 }}>{starters.map(([label, prompt]) => <button key={label} onClick={() => send(prompt)} disabled={loading} style={{ textAlign: "left", padding: 12, border: "1px solid #e1dfda", background: "#faf9f6", borderRadius: 12, cursor: "pointer" }}><b>{label}</b><div style={{ fontSize: 12, opacity: .6, marginTop: 4 }}>{prompt}</div></button>)}</div></section>}

      <div style={{ flex: 1, background: "white", borderRadius: 20, padding: 16, minHeight: 420, overflowY: "auto" }}>

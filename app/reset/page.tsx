...
const steps = [
  {
    label: "REGULATE",
    title: "First: get out of the alarm state.",
    body: "You do not need to solve the problem while your nervous system is activated. Drop your shoulders. Unclench your jaw. Take five slow breaths, making the exhale longer than the inhale.",
    prompt: "For the next minute, your only job is to breathe.",
  },
  {
    label: "FACTS",
    title: "What do you actually know?",
    body: "Separate what is happening from the story your mind is projecting. Facts are observable or already known. Predictions are possibilities, not facts.",
    prompt: "FACTS: What is objectively true right now?",
  },
  {
    label: "CONTROL",
    title: "What is yours to control?",
    body: "You control your actions, preparation, attention, communication and response. You do not control other people's decisions, future outcomes or events already outside your hands.",
    prompt: "CONTROL: What can you actually do about this today?",
  },
  {
    label: "SECOND ARROW",
    title: "Don't add a second problem to the first.",
    body: "The first arrow is the difficult event. The second arrow is the extra suffering created by fighting reality, catastrophizing, rehearsing the worst case, or demanding certainty you cannot have.",
    prompt: "What reaction can you release right now?",
  },
  {
    label: "NEXT MOVE",
    title: "Choose one useful action — or choose to live.",
    body: "If there is a meaningful action, take the smallest useful step. If there isn't, stop trying to solve it and return to your life. Family, exercise, music, work, food, rest and enjoyment are not distractions from life. They are life.",
    prompt: "What is the next good decision?",
  },
];
...
export default function ResetPage() {
  ...
  function restart() {
    setStep(0); setSeconds(60); setStarted(false); setNote(""); setComplete(false);
  }
      ...
      <div style={{ maxWidth: 620, margin: "0 auto" }}>
        ...
        {!complete ? <>
          ...
          <section style={{ marginTop: 14, background: "white", borderRadius: 24, padding: 24, boxShadow: "0 8px 30px rgba(0,0,0,.05)" }}>
            ...
            {(step === 1 || step === 2 || step === 3 || step === 4) && <textarea value={note} onChange={e => setNote(e.target.value)} placeholder={step === 1 ? "Write the facts…" : step === 2 ? "What is one thing you can do?" : step === 3 ? "What can you stop adding to this situation?" : "One next move — or what you choose to enjoy instead…"} style={{ width: "100%", minHeight: 100, marginTop: 14, boxSizing: "border-box", border: "1px solid #ddd", borderRadius: 12, padding: 12, font: "inherit", resize: "vertical" }} />}
